...
<?php
	...
	elseif ($_REQUEST['tab']=='inactive'){
	    $db->order_by="id DESC";
		$contact_detail=$db->get_all('contacts',array('visibility_status'=>'inactive','is_customer'=>'yes'));
	}
	else {
	    $db->order_by="id DESC";
		$contact_detail=$db->get_all('contacts',array('is_customer'=>'yes'));
	}
?>
...
if(isset($_REQUEST['action_delete']))
{
    ...
    {
        $session->redirect('contacts',user);
    }
...
<div class="row">
	...
	$current_tab=$_REQUEST['tab'];?>
		<a href="<?php echo $link->link('contacts',user);?>" class="btn btn-primary"> Customers </a>
	<a href="<?php echo $link->link('suppliers',user);?>" class="btn btn-default"> Suppliers </a>
	<?php if(in_array('create_and_edit',$con_cintact)){?>
	<a href="<?php echo $link->link('import_contact',user);?>" class="btn btn-default pull-right"> Import</a>
	<a href="<?php echo $link->link('add_contact',user);?>" class="btn btn-default pull-right"> Add Customers</a>
 <?php }?>
	</div>
	</div>
	                  <div class="heading tabs">
	                    ...
	                    <ul class="nav nav-tabs pull-left" data-tabs="tabs">
	                      <li class="<?php if ($current_tab=="active"){echo"active";}?>">
	                        <a  href="<?php echo $link->link('contacts',user,'&contact_type='.$tab_name.'&tab=active');?>"><i class="icon-comments"></i><span>Active</span></a>
	                      </li>
	                      <li class="<?php if ($current_tab=="inactive"){echo"active";}?>">
	                        <a  href="<?php echo $link->link('contacts',user,'&contact_type='.$tab_name.'&tab=inactive');?>"><i class="icon-user"></i><span>Inactive</span></a>
	                      </li>
	                      <li class="<?php if ($current_tab=="all" || $current_tab==""){echo"active";}?>">
	                        <a  href="<?php echo $link->link('contacts',user,'&contact_type='.$tab_name.'&tab=all');?>"><i class="icon-user"></i><span>All</span></a>
	                      </li>
	                     </ul>
	                  </div>
					                  ...
					                  <tbody>
					                   <?php
					                    ...
					                    <tr>
					                      ...
					                      <td>
					                       <?php echo $value['business_name'].' '.$value['display_name'];?>
					                      </td>
					                      <td class="hidden-xs">
					                       ...
					                       <td class="hidden-xs">
					                         ...
					                         {
					                             echo "Address : " .$value['postal_address'] ."<br>";
					                         }
					                          if($value['postal_address_town']!='')
					                         {
					                            echo "Town : " .$value['postal_address_town'] ."<br>";
					                         }
					                        if($value['postal_address_suburb']!='')
					                         {
					                             echo "Suburb : " .$value['postal_address_suburb'] ."<br>";
					                         }
					                        if($value['postal_address_state']!='')
					                         {
					                              echo "State : " .$value['postal_address_state'] ."<br>";
					                         }
					                         if($value['postal_address_postcode']!='')
					                         {
					                              echo "Postcode : " .$value['postal_address_postcode'] ."<br>";
					                         }
					                      ...
					                      <td>  <div class="btn-group">
                                            ...
                                            <li>
                                            <a href="<?php echo $link->link('contacts',user,'&action_delete='.$value['id']);?>"><i class="fa fa-edit"></i>Delete</a>
                                              </li>
                                              <?php }?>
                                                <li>
                                                <?php if ($value['visibility_status']=="inactive"){?>
                                                <a href="<?php echo $link->link('contacts',user,'&action_active='.$value['id']);?>"><i class="fa fa-edit"></i>Active</a>
                                                <?php }else{?>
                                                <a href="<?php echo $link->link('contacts',user,'&action_inactive='.$value['id']);?>"><i class="fa fa-edit"></i>Inactive</a><?php }?>
                                              </li>
                                            </ul>
                                          </div></td>
