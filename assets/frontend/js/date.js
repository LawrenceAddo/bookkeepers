...
(function () {
  const $D = Date
  const $P = $D.prototype
  const $C = $D.CultureInfo
  const p = function (s, l) {
    if (!l) {
      l = 2
    }
  ...
  $D.getMonthNumberFromName = function (name) {
    ...
    const m = $C.abbreviatedMonthNames
    const s = name.toLowerCase()
    for (let i = 0; i < n.length; i++) {
  ...
  $P.setTimezoneOffset = function (offset) {
    const here = this.getTimezoneOffset()
    const there = (Number(offset) * -6) / 10
  ...
  $P.getUTCOffset = function () {
    const n = (this.getTimezoneOffset() * -10) / 6
    let r
    if (n < 0) {
      r = (n - 10000).toString()
...
(function () {
  ...
  const dx = 'sunday monday tuesday wednesday thursday friday saturday'.split(
    /\s/
  )
  const mx =
  ...
  const px = 'Millisecond Second Minute Hour Day Week Month Year'.split(/\s/)
  const pxf =
    'Milliseconds Seconds Minutes Hours Date Week Month FullYear'.split(/\s/)
  const nth = 'final first second third fourth fifth'.split(/\s/)
  let de
  $P.toObject = function () {
    const o = {}
  ...
  const ef = function (j) {
    return function () {
      ...
      if (this._same) {
        ...
        const o1 = this.toObject()
        const o2 = (arguments[0] || new Date()).toObject()
        let v = ''
...
(function () {
  ...
  var _ = ($P.Operators = {
    ...
    product: function () {
      ...
      const qx = Array.prototype.slice.call(arguments, 1)
      const rx = []
      for (let i = 0; i < px.length; i++) {
