...
!(function ($) {
  const DateRangePicker = function (element, options, cb) {
    ...
    this.parentEl =
      (hasOptions && options.parentEl && $(options.parentEl)) ||
      $(this.parentEl)
    ...
    if (hasOptions) {
      ...
      if (typeof options.ranges === 'object') {
        for (var range in options.ranges) {
          let start = options.ranges[range][0]
          let end = options.ranges[range][1]
    ...
    this.container
      .find('.ranges')
      .on('click', 'button', $.proxy(this.clickApply, this))
  ...
  DateRangePicker.prototype = {
    ...
    updateFromControl: function () {
      ...
      const start = Date.parseExact(dateString[0], this.format)
      const end = Date.parseExact(dateString[1], this.format)
    ...
    buildCalendar: function (month, year) {
      const firstDay = Date.today().set({ day: 1, month, year })
      const lastMonth = firstDay.clone().add(-1).day().getMonth()
      const lastYear = firstDay.clone().add(-1).day().getFullYear()

      const daysInMonth = Date.getDaysInMonth(year, month)
      const daysInLastMonth = Date.getDaysInMonth(lastYear, lastMonth)
    ...
    renderCalendar: function (calendar, selected, minDate, maxDate) {
      ...
      if (!minDate || minDate < calendar[1][1]) {
        html +=
          '<th class="prev available"><i class="fa.fa-angle-left"></i></th>'
      } else {
        html += '<th></th>'
      }
      html +=
      ...
      if (!maxDate || maxDate > calendar[1][1]) {
        html +=
          '<th class="next available"><i class="fa.fa-angle-right"></i></th>'
      } else {
        html += '<th></th>'
      }
